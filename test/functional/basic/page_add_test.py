...
class PageAddTest(TestCase):
	def test_should_add_a_page_and_redirect_to_root(self):
		...
		response = mocks.app().post('/page/', {'url':url})
		self.assertFalse(page.Page.find(mocks.user, url).error)
		response.follow().mustcontain('Welcome')
		self.assertEqual(response.follow().request.url, mocks.app_root)
	...
	def test_should_add_a_page_and_display_error_for_a_malformed_page(self):
		...
		response = mocks.app().post('/page/', {'url':url})
		self.assertTrue(page.Page.find(mocks.user, url).error)
